...
private func ContentsCheck()
{
  ...
  // Nicht zersägbare empfindliche Objekte über den Ausgang rauswerfen
  if(!fSawable) if(GetDefFragile(GetID(obj)))
  {
    SetCommand(obj, "Exit");
    return();
  }
...
private func Sawing()
{
  ...
  SetAction("Idle");
  // Inhalt auswerfen
  Exit(ContentsNotCrewMember(),-20,14,0,-1);
  return(1);
}
...
private func ContentsNotCrewMember()
{
  var obj;
