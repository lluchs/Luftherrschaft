...
{
  var pObj;
  if (!(pObj = FindObject(0, 0,1,0,0, OCF_Construct))) // Baustelle überlappt Clonk
    pObj = FindObject(0, -16,-16,32,32, OCF_Construct); // Clonk überlappt Baustelle (für noch sehr flache Baustellen)
  return pObj;
}
...
{
  var pObj;
  while (pObj = FindObject(0, 0,1,0,0, OCF_PowerConsumer, "",0, NoContainer(), pObj))
    if (pObj->~NeedsEnergy())
      return pObj;
  return 0;
}
...
{
  return FindObject(0, 0,1,0,0, OCF_Chop);
}
